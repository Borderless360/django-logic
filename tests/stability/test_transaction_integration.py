...
@tag('stability')
class TestUtilsRestoration(StabilityTestCase):
    """
    Tests for the worker-side restoration utilities that would be used
    by the Celery handler in BackgroundTransition.
    """

    def test_restore_action_finds_transition(self):
        ...
        self.assertEqual(transition.action_name, 'fulfill')
        self.assertEqual(transition.target, 'fulfilled')
        self.assertEqual(process.state.instance.pk, order.pk)

    def test_restore_action_with_ignore_sources(self):
        """
        When the instance is already in in_progress_state, restore_action
        should still find the transition (via ignore_sources=True).
        """
        from django_logic.utils import restore_action
    ...
    def test_get_process_and_state(self):
        from django_logic.utils import get_process_and_state
        ...
        self.assertEqual(state.get_state(), 'approved')
        self.assertEqual(state.instance.pk, order.pk)
